...
import { CONFIG_DIR_NAME, type ExtensionAPI, getMarkdownTheme } from '@earendil-works/pi-coding-agent'
import { Container, Markdown, Spacer, Text } from '@earendil-works/pi-tui'
import { Type } from 'typebox'

const SUBAGENT_ENV = 'PI_TASK_SUBAGENT'
const LOCKDOWN_SUBAGENT_ENV = 'PI_LOCKDOWN_SUBAGENT'
...
function readConfigModel(cwd: string): string | undefined {
  try {
    ...
    return typeof subagentTask?.model === 'string' && subagentTask.model.trim() ? subagentTask.model.trim() : undefined
  } catch {
    return undefined
  }
}
  ...
  try {
    ...
    const exitCode = await new Promise<number>((resolve) => {
      const proc = spawn(invocation.command, invocation.args, {
        ...
        env: { ...process.env, [SUBAGENT_ENV]: '1', [LOCKDOWN_SUBAGENT_ENV]: '1' },
      })
      let buffer = ''
      ...
      const processLine = (line: string) => {
        ...
        if (event.type === 'message_end' && event.message) {
          ...
          if (ctx.hasUI) {
            ...
            ctx.ui.setWidget(
              'task',
              preview ? ['Running subagent...', `Task: ${task}`, preview] : ['Running subagent...', `Task: ${task}`],
            )
          }
        }
      }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerMessageRenderer('task-result', (message, { expanded, outputPad }, theme) => {
    ...
    if (isError && details.errorMessage) {
      container.addChild(new Text(theme.fg('error', `Error: ${details.errorMessage}`), outputPad, 0))
    }
    ...
    if (expanded) {
      ...
      container.addChild(new Text(theme.fg('toolOutput', lines.slice(0, 10).join('\n')), outputPad, 0))
      if (lines.length > 10) {
  ...
  pi.registerCommand('subagent-task', {
    description: 'Spawn a subagent to complete a task (inherits extensions, permissions, and settings)',
