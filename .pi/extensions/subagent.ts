...
function formatToolCall(
  ...
  themeFg: (_: unknown, text: string) => string,
): string {
  ...
  try {
    ...
    const exitCode = await new Promise<number>((resolve) => {
      ...
      const proc = spawn(invocation.command, invocation.args, {
        ...
        env: { ...process.env, PI_LOCKDOWN_SUBAGENT: '1' },
      })
      let buffer = ''
...
const AgentScopeSchema = StringEnum(['user', 'project', 'both'] as const, {
  description: 'Which agent directories to use. Default: "project". Use "both" to include project-local agents.',
  default: 'project',
})
